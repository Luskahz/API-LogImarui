import { baseModelMap } from "./baseRouterMap.js"
...
export function buildBaseMetadata(baseId, data, filename, uploader = "Developer") {
  // Pega os campos do modelo Prisma dinamicamente
  const modelDelegate = baseModelMap[baseId]
  if (!modelDelegate) throw new Error(`baseId ${baseId} não encontrado em baseModelMap`)

  // Pega os campos do modelo Prisma
  const modelFields = Object.keys(modelDelegate.fields || {})
